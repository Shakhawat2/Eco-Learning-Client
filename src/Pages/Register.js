...
const Register = () => {
    const { createAccount, updateUser, google, github } = useContext(AuthContext);
    ...
    const loginwithGithub = () => {
        github()
            .then((result) => {
                toast.success('Successful log in by github');
                navigate('/')
            }).catch((error) => {
                // Handle Errors here.
                const errorCode = error.code;
        ...
        <div className="my-10 w-full mx-auto max-w-md p-8 space-y-3 rounded-xl dark:bg-gray-900 dark:text-gray-100">
            ...
            <form onSubmit={handleRegister} noValidate="" action="" className="space-y-6 ng-untouched ng-pristine ng-valid">
                ...
                <div className="space-y-1 text-sm">
                    ...
                    <input type="email" name="email" id="email" placeholder="Email" className="border border-2 w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" required />
                </div>
                <div className="space-y-1 text-sm">
                    <label htmlFor="password" className="block dark:text-gray-400">Password</label>
                    <input type="password" name="password" id="password" placeholder="Password" className="border border-2 w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" required />
                </div>
